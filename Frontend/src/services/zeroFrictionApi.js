...
export const getNotes = async (sessionId, language = 'English') => {
  const endpoint = `/notes/${sessionId}?lang=${encodeURIComponent(language)}`;
...
export const getQuiz = async (sessionId, language = 'English') => {
  const endpoint = `/quiz/${sessionId}?lang=${encodeURIComponent(language)}`;
  const payload = await request(endpoint);
  if (payload) return assertSuccess(payload, endpoint);
...
export const submitAnswer = async (sessionId, answer, language = 'English') => {
  const endpoint = `/submit/${sessionId}?lang=${encodeURIComponent(language)}`;
  const payload = await request(endpoint, {
    method: 'POST',

...
const Notes = () => {
  const { sessionId: contextSessionId, transcript, notes, setNotes, setQuizQuestions, setPipelineStep, pipelineStep, language } = useAppContext();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const notesData = await getNotes(sessionId, language);
        setNotes(notesData);
        const quizData = await getQuiz(sessionId, language);
  ...
  }, [sessionId, language, setNotes, setQuizQuestions, navigate, pipelineStep]);

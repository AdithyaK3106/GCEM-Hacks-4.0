...
export const AppProvider = ({ children }) => {
  ...
  const translateCurrentContent = async (newLang) => {
    if (!sessionId) return;
    
    try {
      // Re-fetch notes and quiz in the new language
      const { getNotes, getQuiz } = await import('../services/zeroFrictionApi');
      const notesData = await getNotes(sessionId, newLang);
      setNotes(notesData);
      const quizData = await getQuiz(sessionId, newLang);
  ...
  useEffect(() => {
    if (sessionId && language) {
      translateCurrentContent(language);
    }
  }, [language]);
